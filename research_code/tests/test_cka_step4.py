import numpy as np
import unittest
from research_code.cka_step4 import cka, cka_tall, cka_wide
from research_code.cka_step3 import cka as old_cka
...
class TestCka(unittest.TestCase):
    ...
    def test_consistent(self):
        """Regression test: check that the old implementation gives the same 
        results as the new implementation."""
        X, Y = _get_wide()
        c1 = cka(X, Y)
        c2 = old_cka(X, Y)
    ...
    def test_tall_wide(self):
        """Check that both implementations gives the same results"""
        X, Y = _get_wide()
        c1 = cka_tall(X, Y)
        c2 = cka_wide(X, Y)
